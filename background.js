function scheduleOpenSettingsModal(tabId) {
  setTimeout(() => {
    chrome.tabs.sendMessage(tabId, { action: "OPEN_SETTINGS_MODAL" }).catch(e => console.error(e));
  }, 100);
}
...
function openSettingsModal(tabId) {
  chrome.tabs.sendMessage(tabId, { action: "OPEN_SETTINGS_MODAL" }).catch((err) => {
    console.log("Orphaned tab detected or content script uninitialized. Injecting manually...", err);
